...
# Try to get existing repo if not creating a new one
data "github_repository" "existing_repo" {
  count     = (var.enable_github_actions && !var.create_repository) ? 1 : 0
...
resource "github_repository" "repo" {
  ...
  description = "Blueprint Agent CI/CD Repository"
  visibility  = "private"
...
locals {
  ...
  repo_dependency = (
    var.enable_github_actions ? (
      var.create_repository ? [github_repository.repo[0]] : [data.github_repository.existing_repo[0]]
    ) : []
  )
}
...
resource "github_actions_secret" "wif_pool_id" {
  ...
  plaintext_value = google_iam_workload_identity_pool.github_pool[0].workload_identity_pool_id
  depends_on      = [github_repository.repo, data.github_repository.existing_repo]
}
...
resource "github_actions_secret" "wif_provider_id" {
  ...
  plaintext_value = google_iam_workload_identity_pool_provider.github_provider[0].workload_identity_pool_provider_id
  depends_on      = [github_repository.repo, data.github_repository.existing_repo]
}
