...
A2A_RPC_URL = BASE_URL + "/a2a/blueprint_agent/"
...
def start_server() -> tuple[subprocess.Popen[str], list[threading.Thread]]:
    ...
    command = [
        ...
        "-m",
        "uvicorn",
        "blueprint_agent.fast_api_app:app",
        "--host",
        "0.0.0.0",
        "--port",
        "8000",
...
def test_adk_run_sse(server_fixture: subprocess.Popen[str]) -> None:
    ...
    session_response = requests.post(
        f"{BASE_URL}/apps/blueprint_agent/users/{user_id}/sessions",
        headers=HEADERS,
        json=session_data,
        timeout=60,
    )
...
def test_reasoning_engine_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    This is the contract Agent Runtime forwards :streamQuery calls to.
    """
    response = requests.post(
